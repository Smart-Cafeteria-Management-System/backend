...
func Seed(db *gorm.DB) error {
	...
	// Create meal slots for today and tomorrow
	...
	// ==========================================
	// SEED DEMAND FORECASTS (for Epic 4)
	// ==========================================
	log.Println("Seeding demand forecasts...")
	...
	// Past 14 days with actual data (for accuracy calculation)
	for i := 14; i >= 1; i-- {
		date := today.AddDate(0, 0, -i)
		dayIdx := i % 7
	...
	// Future 7 days predictions (no actual data yet)
		...
		wasteLogs := []models.WasteLog{
			{
				...
				RecordedBy:       staff.ID,
				WeatherCondition: weathers[i%7],
				AcademicSchedule: schedules[i%7],
			},
			{
				Date:             date,
				MealType:         models.MealDinner,
